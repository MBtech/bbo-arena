...
from utils import getAll, parseLogsAll
import sys
import numpy as np
...
for system in config["systems"]:
    for app in config["applications"][system]:
        for datasize in config["datasizes"]:
            ...
            data = pd.DataFrame(runtimes, columns = ['Algorithms', 'Budget', 'Runtime', 'Experiment', 'Type', 'Size', 'Num'])
            data['Family'] = data['Type'] +'.'+ data['Size']
            d = data.groupby('Algorithms')
            for group, df in d:
                print(group)
                ax.plot(df['Family'], df["Num"], label=group)
